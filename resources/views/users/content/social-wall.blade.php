...
@section('content')
    <main class="relative" x-data="{ ad: false, card: false }">
        <section class="page-title bg-blue-550 h-36 lg:h-60 bg-center bg-cover"
            style="background-image: url({{ asset('assets/Group-397.png') }})">
            <div class="w-full mx-auto flex overflow-hidden flex-col justify-between h-full">
                ...
                <h3 class="text-white py-4 text-center text-2xl md:text-4xl lg:text-9xl uppercase">
                    <marquee>{{ $event->wall_location_msg ?? '' }}</marquee>
                </h3>
            </div>
        </section>
            ...
            <div class="clearfix" id="fh5co-board" data-columns>
                @if (count($posts) > 0)
                    ...
                    @foreach ($posts as $post)
                        ...
                        <div class="item">
                                <!--<a href="javascript:void(0)" class="fh5co-board-img post-item">-->
                                <div @click=" card = true " class="fh5co-board-img post-item cursor-pointer">
                                    <div class="relative">
                                        @if ($post->platform != 'tiktok')
                                            @php
                                                if (file_exists($post->image)) {
                                                    ...
                                                    $path = 'https://endodonticsonly.com.au/wp-content/uploads/placeholder.png';
                                                }
                                            @endphp
                                            <img class=" rounded-lg w-full object-cover" src="{{ $post->image }}"
                                                alt="Dummy Image">
        ...
        <section class="absolute w-full h-screen top-0 left-0 bg-black bg-opacity-80" x-show="ad" @click.away="ad = !ad"
            ...
            <div class="h-full w-full flex flex-wrap justify-center items-center">
                <div class="w-9/12 mx-auto relative rounded-xl">
                    ...
                    <p class="absolute left-5 top-4"><i class="far fa-clock"></i><span class="text-sm pl-2">This Ad will
                            close in 12 seconds.</span></p>
        ...
        <section
            class="fixed w-full h-full top-0 left-0 bg-black bg-opacity-80 flex flex-wrap items-center justify-center"
            x-show="card" @click.away="card = !card"
            x-transition:enter="transition transform origin-top-left ease-out duration-300"
            x-transition:enter-start="opacity-0 scale-75" x-transition:enter-end="opacity-100 scale-100"
            x-transition:leave="transition transform origin-top-left ease-out duration-300"
            x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-75">
            <div class="w-1/2 mx-auto flex flex-wrap overflow-hidden rounded-xl">
                <div class=" overflow-hidden w-1/2">
                    <img src="{{ asset('assets/Maroon-5-2018-web-optimised-1000.jpg') }}"
                        class=" w-full h-full object-cover" alt="">
                </div>
                <div class=" overflow-hidden w-1/2 bg-white p-5 relative">
                    <p class="absolute right-5 top-4"><i class="fas fa-times cursor-pointer" @click=" card = !card "></i>
                    </p>
                    <div class="flex flex-wrap overflow-hidden justify-start items-center">
                        <img src="{{ asset('assets/sample-profile.png') }}"
                            class="w-10 h-10 rounded-full object-contain bg-white avatar" alt="">
                        <div class="pl-2">
                    ...
                    <p class="py-5">
                        Weekend sorted 🥳 Take the family out for a day of movie madness with $8* TICKETS on select family
                        films! Book your tickets now 👉 bit.ly/SaveFamilyTix
                        <br>
                        *$1.50 online booking fees apply. Must be a Cinebuzz member to access special price. Excludes
                        premium seating options.
                    </p>
                    <p class="tags">
                        #event #wedding #party #events #eventplanner #music #love #photography #birthday #dj #art
                        #eventorganizer #weddingplanner #instagood #like #fun #fashion #design #instagram #eventplanning
                        #festival
                    </p>
