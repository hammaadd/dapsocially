...
<main>
    ...
    <section class="py-10 max-w-5xl mx-auto">
        <form action="{{route('add.event')}}" method="POST" enctype="multipart/form-data" onsubmit="return checkInputBox()">
                        ...
                        <div class="w-full overflow-hidden md:my-2 md:px-2 lg:my-3 lg:px-3 xl:my-3 xl:px-3">
                            <label for="cover">
                                ...
                                <div class="w-full h-60 relative border-gray-200 border bg-gray-200 rounded-md mt-1">
                                    <img src="{{asset('assets/Rectangle 119.png')}}" id="coverImage" class="object-cover w-auto rounded-md mx-auto h-full" alt="">
            ...
            <div class="w-full overflow-hidden md:my-2 md:px-2 lg:my-3 lg:px-3 xl:my-3 xl:px-3" x-data="{isPkg:'Free', message:''}">
                ...
                <div class="flex flex-wrap overflow-hidden flex-col items-center p-4 shadow-md rounded-xl bg-white" :class="{'bg-blue-450': isPkg === {{$plans->id}} }">
                    ...
                    <hr class="w-3/5 mx-auto border-gray-900 my-3" :class="{'border-white': isPkg === {{$plans->id}} }">
                    <p class="text-center" :class="{'text-white': isPkg === {{$plans->id}} }">
                        {{$plans->description}}
                    </p>
                    @if ($plans->price == 0)
                    <p class="text-2xl text-center uppercase my-3 font-medium" :class="{'text-white': isPkg === {{ $plans->id }} }">Free</p>
                    @else
                    <p class="text-2xl text-center uppercase my-3 font-medium" :class="{'text-white': isPkg === {{ $plans->id }} }">${{ $plans->price }}
                    </p>
                    @endif
                    <p class="bg-transparent text-blue-550 px-4 py-1.5 border-2 border-blue-550 rounded-3xl hover:text-white hover:bg-blue-550 " :class="{'hover:border-white hover:text-white bg-white': isPkg === {{$plans->id}} }" @click="isPkg = {{$plans->id}}, message='{{$plans->id}}' ">Choose</p>
